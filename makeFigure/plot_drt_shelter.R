...
for (i in seq_along(species.vec)) {
  ...
  dat.sum.wide$drt.gcc <-  dat.sum.wide$GCC.Drought /dat.sum.wide$GCC.Control 
  
  dat.sum.wide$drt.iri <-  dat.sum.wide$irrig.tot.Drought /dat.sum.wide$irrig.tot.Control 
...
pace.effect.df$species <- as.factor(pace.effect.df$species)

pdf('figures/plot.drt.shelter.pdf',width = 8,height = 8*2*.618)
 
plot(drt.gcc~species,data = pace.effect.df)
